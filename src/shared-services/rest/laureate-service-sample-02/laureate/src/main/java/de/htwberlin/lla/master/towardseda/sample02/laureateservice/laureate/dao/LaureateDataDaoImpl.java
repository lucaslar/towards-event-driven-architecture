package de.htwberlin.lla.master.towardseda.sample02.laureateservice.laureate.dao;
...
public class LaureateDataDaoImpl implements LaureateDataDao {
    ...
    @Override
    public Future<List<Laureate>> searchLaureates(final String scope, final JsonObject data) {
        final String from = "laureate l INNER JOIN prize_to_laureate ptl ON l.id = ptl.laureate INNER JOIN prize_category c ON c.id = ptl.category";
        final String query = "SELECT DISTINCT l.* FROM " + from + " WHERE " + buildQueryCondition(data) + " AND c.title='" + scope + "'";
